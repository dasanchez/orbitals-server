...
async def test_hub_player():
    ...
    # JOIN THE BLUE TEAM,
    # AND REQUEST THE HUB ROLE
    print("Waiting for 'Player' to join the blue team and request the hub role...")
    ...
    while srv._tm._table._players["Player"][1] != "hub":
        await asyncio.sleep(0.1)
    ...
    print("Waiting for player to request start...")
    # CLIENT MUST REQUEST START
    status = srv._tm._table.status()
    while status["game_state"] == "WAITING_START":
        status = srv._tm._table.status()
        await asyncio.sleep(0.1)
    print("Player has requested start")

    print("Waiting for player to submit clue (timer should be running)...")
    # CLIENT MUST SUBMIT CLUE
    status = srv._tm._table.status()
    ...
    print("Waiting for player to approve clue (timer should be stopped)...")
    ...
    # print("Waiting for player to request replay...")
